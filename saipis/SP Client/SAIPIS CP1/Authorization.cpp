...
bool Authorization::authorization(SOCKET S, Authorization * autho, string pathFile, string value)
{
	...
	if (res == "true") {
		green cout << "Авторизация прошла успешно\n" << value << endl; white
		return true;
	}
	else {
		red cout << "Неверно введён логин или пароль" << endl; white
		return false;
	}
}
...
void Authorization::registration(Authorization * autho, string pathFile, string value)
{
	...
	red		cout << "База данных не найдена" << endl; white
			cout << "Идет создание новой" << endl;
	green	cout << "База данных успешно создана" << endl; white
