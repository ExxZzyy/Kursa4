		...
		if (str[0] == acceptedSymbolsToUpperCase[count1]) {
			for (int i = 1; i < str.length(); i++) {
				for (int j = 0; j < symbols.length(); j++) {
					if (symbols[j] == str[i]) {
						red cout << i << "ѕерва€ буква не может быть символом" << endl; white
							return false;
					}
				}
				if (str[i] >= '0' && str[i] <= '9') {
					red cout << i << "ѕерва€ буква не может быть числом" << endl; white
						return false;
				}
				if ((str[i] >= 'a' && str[i] <= 'z') || (str[i] >= 'A' && str[i] <= 'Z')) {
					cout << i << "ѕерва€ буква должна быть из кириллицы" << endl;
					return false;
				}
			...
			if (count2 != str.length() - 1) return true;
				...
				red cout << "¬тора€ и последующие буквы должны быть нижнего регистра" << endl; white
					return false;
			}
		}
		else {
			red cout << "ѕерва€ буква должна быть верхнего регистра" << endl; white
				return false;
		}
	}
	else {
		cout << "ѕоле должно состо€ть не менее, чем из двух символов" << endl;
		return false;
	}
}
